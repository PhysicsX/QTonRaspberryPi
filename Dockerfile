...
RUN { \
    ...
    wget https://download.qt.io/official_releases/qt/6.11/6.11.0/submodules/qtshadertools-everywhere-src-6.11.0.tar.xz && \
    wget https://download.qt.io/official_releases/qt/6.11/6.11.0/submodules/qtdeclarative-everywhere-src-6.11.0.tar.xz && \
    cd ../host-build && \
    tar xf ../src/qtbase-everywhere-src-6.11.0.tar.xz && \
    tar xf ../src/qtshadertools-everywhere-src-6.11.0.tar.xz && \
    tar xf ../src/qtdeclarative-everywhere-src-6.11.0.tar.xz && \
    ...
    cd ../qtshadertools-everywhere-src-6.11.0 && \
    /build/qt6/host/bin/qt-configure-module . && \
    cmake --build . --parallel 4 && \
    cmake --install . && \
    echo "Compile declerative for host" && \
    cd ../qtdeclarative-everywhere-src-6.11.0 && \
    /build/qt6/host/bin/qt-configure-module . && \
    cmake --build . --parallel 4 && \
    cmake --install . && \
    cd ../../pi-build && \
